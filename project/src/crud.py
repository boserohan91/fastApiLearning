...
async def get_all_blogs(ses: Session):
    return ses.query(models.Blogs).all()

async def get_blog_by_id(ses: Session, id: int):
    return ses.query(models.Blogs).filter(models.Blogs.id == id).first()

async def delete_blog_by_id(ses: Session, id: int):
...
async def get_blog_by_author(ses: Session, author: str):
    return ses.query(models.Blogs).filter(models.Blogs.user.has(models.Users.name==author)).all()

async def create_blog(ses: Session, blog: schemas.BlogCreate):
...
async def create_user(ses: Session, user: schemas.User):
...
async def get_user(ses: Session, email: str):
